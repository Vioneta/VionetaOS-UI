<template>
	<div :class="{ '_max-width-320': currentStep === 1 || currentStep === 3 || currentStep === 4 }" class="modal-card">
		...
		<header :class="{ 'pri-head': currentStep <= 2 }" class="modal-card-head">
			<b-icon
				v-if="currentStep === 1"
				class="mr-2 has-text-red"
				custom-class="_has-text-red-default"
				icon="danger"
				pack="casa"
				size="is-20"
			></b-icon>
			...
			<div v-for="(item, index) in storageData" :key="item.path + index" class="is-flex mb-1 radius _height-40">
				...
				<div class="is-flex is-flex-shrink-0 is-flex-direction-column is-justify-content-center mr-2">
					<span class="is-uppercase _is-text-full-03 _has-text-gray-600"
						>{{ renderSize(item.size - item.availSize) }}/{{ renderSize(item.size) }}</span
					>
				</div>
				<b-checkbox
					v-model="checkBoxGroup"
					:disabled="item.persistedIn !== 'vionetaos' || item.isSystem"
					:native-value="item.uuid"
					class="mr-2"
				></b-checkbox>
			</div>

			<div
				v-for="(item, index) in storageMissData"
				:key="item.path + index"
		...
		<div
			v-if="currentStep === 0 && storageData.length < 2"
		...
		<div
			v-if="currentStep === 0 && checkBoxGroup.length > 0"
			class="_has-background-yellow-default _has-text-white _is-normal is-flex is-align-items-center font ml-5 mr-5 mb-4 pt-2 pb-2 _radius-line"
		>
			<div class="is-flex left ml-3 mr-2 is-align-items-center">
				<b-icon class="is-16x16" custom-size="casa-19px" icon="danger" pack="casa"></b-icon>
		...
		>
			...
			<template v-if="currentStep === 2">
				...
				<b-input
					...
					class="mt-4"
					password-reveal
					@keyup.enter.native="verifyOperate(password)"
				></b-input>
			</template>
		...
		<footer :class="{ 't-line': currentStep <= 2 }" class="modal-card-foot is-flex is-align-items-center">
			...
			<div class="mr-4">
				<b-button
					v-show="currentStep > 2 || currentStep === 1"
					:label="$t('Cancel')"
...
export default {
	...
	computed: {
		...
		isSplit() {
			return !this.mergeStorageList.every(
				(item) => this.checkBoxGroup.includes(item) || this.storageMissData.includes(item)
			);
		},
	},
	data() {
	...
	methods: {
		...
		updateMerge(dockerInfo) {
			...
			this.$api.local_storage
				.updateMergerfsInfo({
					...
					mount_point: "/DATA",
					source_volume_uuids: [...this.checkBoxGroup, ...this.checkBoxMissGroup],
				})
				.then((res) => {
		...
		async getMerageStorage() {
			try {
				...
				this.mergeInfo = await this.$api.local_storage
					.getMergerfsInfo()
		...
		async test() {
			...
			this.notEmpty = await this.$api.folder
				.getFolderSize("/DATA")
				.then((res) => {
		...
		async submit(e, nextStep = false) {
			...
			let notSplit = this.mergeStorageList.every(
				(item) => this.checkBoxGroup.includes(item) || this.checkBoxMissGroup.includes(item)
			);
