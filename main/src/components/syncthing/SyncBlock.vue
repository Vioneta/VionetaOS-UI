<template>
	<div class="common-card">
		...
		<div class="wuji-content _box is-flex is-flex-direction-column">
			<!-- Init State Start -->
			<h6 class="title is-4 mb-0 has-text-white is-flex-shrink-0">
				{{ $t(`Create meaningfull inside from data`) }}
			</h6>
			<div class="is-flex is-align-items-center is-flex-grow-1 _notice-content-text">
				<div class="info">
					<div class="des two-line is-size-14px">
						{{ $t(`VInsight is Vioneta's Advanced Analytical engine `) }}
					</div>
				</div>
				<b-image :src="require('@/assets/img/syncthing/icon.png')" class="is-80x80"></b-image>
			</div>
...
export default {
	...
	methods: {
		async checkSyncStatus() {
			...
			const systemApps = listRes.data ? listRes.data.data.vionetaos_apps : [];
