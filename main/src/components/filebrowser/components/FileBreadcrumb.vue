...
<template>
	<div>
		<b-breadcrumb size="is-medium">
			<b-breadcrumb-item
				v-for="(item, index) in pathCollection"
				v-show="item.show || item == activeHide"
				:key="item + index"
			...
			>
				<template v-if="item == activeHide">
					...
					>
						...
						<b-dropdown-item
							v-for="(sitem, index) in hideItems"
							:key="'ff' + index"
							aria-role="menuitem"
							@click="subOpen(sitem)"
						>
			...
			this.$nextTick(() => {
				...
				if (shadowBreadWidth > containerWidth) {
					...
					this.pathCollection.forEach((item, index) => {
						if (
							index > 0 &&
							sum(shadowBreadListWidth) + 50 > containerWidth &&
							index < this.pathCollection.length - 1
						) {
