...
<template>
	...
	>
		...
		<h1 v-if="status === 'pending'" class="has-text-sub-03 has-text-white mt-6">
		...
		<span v-else class="has-text-full-03 has-text-grey-600 mt-6"
			>{{ $t("Please") }} <a @click="openThirdApp(appDetailData)">{{ $t("Click here") }} </a>
			{{ $t("to open the app. If it does not work, please restart or try again later.") }}
		</span>
...
export default {
	...
	methods: {
		...
		async startContainer() {
			try {
				let res = await this.$openAPI.appManagement.compose.setComposeAppStatus(
					this.appDetailData.name,
					"start"
				);
				return res.data;
			} catch (error) {
